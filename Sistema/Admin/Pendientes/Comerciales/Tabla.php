				...
				<div class="form-group col-md-12" >
					...
					<table class="table table-bordered">
						...
						<tbody>
							...
							<?php
								$vSQL="exec web.SP_LISTADO_SERVICIOS_ADMIN 3,$num_sol";
									...
									{			
											...
											if($FG_PRELIQUIDA==0)
										{
											$BOTONES =$BOTONES.'<div class="btn-toolbar" role="toolbar">
														...
														<button type="button" data-toggle="tooltip" title="Preliquidar" disabled class="btn btn-primary">
										...
										else
										{
											...
											$BOTONES =$BOTONES.'<div class="btn-toolbar" role="toolbar">
														<div class="btn-group">
															<button type="button" data-toggle="tooltip" title="Preliquidar"  class="btn btn-primary" onClick="generar_preliq('.$num_sol.',\''.$NB_PROCEDURE.'\',\''.$CONECTOR.'\',\''.$FORMATO_PRELIQ.'\');">
																<span class="glyphicon glyphicon-usd"></span>
															</button>
...
<script>
	...
	function generar_preliq(id_solic, nb_procedure, conector, formato, stipos, agente, rif)
		{
				
			Parametros="consulta=1&solicitud="+id_solic+"&nb_procedure="+nb_procedure+"&conector="+conector+"&formato="+formato+"&stipo="+stipos+"&agente="+agente+"&rif="+rif;
			...
			$.ajax(
			{
				type: "POST",
				dataType:"html",
				url: "Sistema/Admin/Pendientes/Comerciales/ScriptGenPre.php",			
				data: Parametros,	
				beforeSend: function() 
				{
	...
	?>
						...
						{						
							...
							if($id>0)
							{
								...
								$("#consulta").load('Sistema/Admin/Pendientes/Comerciales/Tabla.php?id='+id_solic);
							}
							else
							{
								window.parent.Cargando(0);
