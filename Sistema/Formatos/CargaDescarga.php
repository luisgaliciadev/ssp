     ...
     $MSJ_ERROR=$ResultadoEjecutar["MSJ_ERROR"];
      $result=$ResultadoEjecutar["RESULTADO"];
       ...
       while ($registro=odbc_fetch_array($result))
       {
		  ...
		  $DESCRIP = utf8_encode(odbc_result($result,'DESCRIP')); 
     ...
     $MSJ_ERROR=$ResultadoEjecutar["MSJ_ERROR"];
      $result=$ResultadoEjecutar["RESULTADO"];
       ...
       while ($registro=odbc_fetch_row($result))
       {
		  ...
		  $DESCRIP_SERVICIO = utf8_encode(odbc_result($result,'DESCRIP_SERVICIO')); 
...
$Conector=Conectar2();
	$sql = "select * from web.View_Web_Rpt_SolMuelle where id = $ID_SOLIC_MUELLE";
	 $ResultadoEjecutar=$Conector->Ejecutar($sql, $INSERT_MAYUSCULA="SI", $SP_BITACORA='NO', $SP_ACCION='', $SP_NB_TABLA='', $SP_VALOR_CAMPO_ID='');
	 $CONEXION=$ResultadoEjecutar["CONEXION"];						
     $ERROR=$ResultadoEjecutar["ERROR"];
     $MSJ_ERROR=$ResultadoEjecutar["MSJ_ERROR"];
...
<table  border="1" width="100%" >
		<tr>
			 ...
			 <div align="center"> 
			 	<h2>PRELIQUIDACION POR SERVICIO DE ESTIBA</h2>
				</div>
			 </td>
			 </div>
...
<table border=1 class="margenes">
    ...
    <tbody>
        <tr>
            ...
            <td>'.$ANO_EJERCICIO.' - '.$nro_solicitud.'</td>
...
$mpdf->SetTitle('Servicios de Estiba');
